...
import NoteData from "@/data/notesData.json"
...
//   高亮
  if (!tocTitleDivList.value || !tocTitleDivList.value.length) {
    tocTitleDivList.value = document.getElementsByClassName("md-editor-catalog-link")! as HTMLCollectionOf<HTMLElement>
  }
...
const getContext = async (id: string) => {
  ...
  for (const item of (NoteData as Array<{ id: number, title: string }>)) {
    if (item.id.toString() === id) {
      console.log("文件路径：", `${import.meta.env.BASE_URL}notes/${item.title}.md`)
